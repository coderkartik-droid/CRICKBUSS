...
CONTACT_SUBJECT_CHOICES = [
    ('General Inquiries & Feedback', 'General Inquiries & Feedback'),
    ('Advertising & Sponsorship', 'Advertising & Sponsorship'),
    ('Editorial Tips & Corrections', 'Editorial Tips & Corrections'),
    ('Technical Issue / Bug Report', 'Technical Issue / Bug Report'),
]


class ContactForm(forms.ModelForm):
    """Public Contact Us form — stores the message for the admin inbox."""
    ...
    class Meta:
        ...
        widgets = {
            ...
            'subject': forms.Select(attrs={'class': 'form-select'}, choices=CONTACT_SUBJECT_CHOICES),
...
class SimpleVideoUploadForm(forms.ModelForm):
    """Dashboard video management form — creates/edits CricketVideo records.

    Only two fields are shown: Title and Video File.
    Everything else is handled automatically:
      - slug        — auto-generated by CricketVideo.save()
      - category    — auto-assigned to "Highlights" (created if missing)
      - is_active   — always set to True
      - thumbnail   — generated automatically from the uploaded video's first frame
      - duration    — calculated from the uploaded video's media metadata
    """
    ...
    def save(self, commit=True):
        """Save with automatic defaults for category and is_active."""
        video = super().save(commit=False)
        # Auto-assign "Highlights" category (create if it doesn't exist)
        if not video.category_id:
            default_cat, _ = VideoCategory.objects.get_or_create(
                name='Highlights',
                defaults={'slug': 'highlights', 'description': 'Match highlights and video clips.'}
            )
            video.category = default_cat
